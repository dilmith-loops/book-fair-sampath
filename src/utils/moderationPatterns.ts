...
export function isNonsensicalText(text: string): { isNonsense: boolean; detail?: string } {
  ...
  for (const word of words) {
    ...
    if (cleanWord.length >= 4) {
      ...
      // Normalize valid Sinhala digraphs (th, ch, dh, sh, kh, gh, bh, ph, ng, nd, mb, etc.)
      // and geminates (thth, chch, dhdh, shth, ndr, kkh, tth, nth, nch, ndh, mbh)
      // and common English clusters (str, spl, thr, sch, psych, ght, etc.) into single units
      ...
      // If after normalizing valid digraphs and phonemes, there are 5+ consecutive consonants (e.g. "fjskdfgh")
      ...
      // Word is long (>= 8) but has extremely low vowel count (<= 1 vowel in 8+ letters, e.g. "kjsdhfgk")
...
export const ENGLISH_PROFANITY: string[] = [
  ...
  'asshole', 'bastard', 'cunt', 'dick', 'pussy', 'pussies', 'cock', 'whore',
  'slut', 'motherfucker', 'jackass', 'douchebag', 'wanker', 'twat', 'prick'
];

// 2. Sinhala Script Profanities and Vulgar Slurs (සිංහල අසභ්‍ය වචන)
export const SINHALA_SCRIPT_PROFANITY: string[] = [
  'පකයා', 'පක', 'පකෝ', 'පකෝ', 'කැරියා', 'කැරි', 'කැරියෝ', 'කැරියෙක්',
  'හුත්ත', 'හුත්තෝ', 'හුත්තේ', 'හුකපන්', 'හුකන්නා', 'හුකන', 'වේසි',
  'වේසාවා', 'වේසිගෙ', 'වේස', 'පොන්නයා', 'පොන්න', 'පොන්ස්', 'බල්ලා',
  'බැල්ලි', 'පරයා', 'පරයෝ', 'කලවැද්දා', 'පයිය', 'බිජ්ජ', 'තොත්තා',
  'අම්මටහුකන', 'අපතයා'
];

// 3. Singlish (Romanized Sinhala Colloquial Slang & Vulgarities)
export const SINGLISH_PROFANITY: string[] = [
  'pakaya', 'pakayaa', 'pakayo', 'paka', 'pako', 'pakoo', 'pake', 'pakenda',
  'kariya', 'kariyo', 'kari', 'kariyek', 'kari balla', 'kariweda',
  'hutta', 'hutto', 'hutte', 'huththe', 'huththa', 'hukapan', 'hukanna', 'hukanawa',
  'wesa', 'wesi', 'wesige', 'wesikema', 'wesigeputha', 'wesige putha', 'wesikeli',
  'ponnaya', 'ponna', 'ponz', 'ponnay', 'ponnayo', 'ponnayek',
  'balla', 'balli', 'ballo', 'paraya', 'parayo', 'kalawadda', 'payya', 'bijja',
  'ammata hukanwa', 'ammata hukanna', 'thuk nodokin', 'pala hutto', 'pala pakaya'
];

// 4. Tamil Script Profanities and Vulgarities (தமிழ் அவதூறு வார்த்தைகள்)
export const TAMIL_SCRIPT_PROFANITY: string[] = [
  'தேவிடியா', 'பூல்', 'ஓத்தா', 'சுன்னி', 'மயிர்', 'பொட்டை', 'புண்டை',
  'கூதி', 'நாயே', 'பன்னி', 'தாயோளி', 'கண்டாரோளி'
];

// 5. Tanglish (Romanized Tamil Vulgarities & Slang)
export const TANGLISH_PROFANITY: string[] = [
  'thevidiya', 'thevdia', 'thevdiya', 'thevidiya paiya', 'otha', 'othale',
  'ommale', 'poolu', 'pool', 'sunni', 'sunniya', 'mayiru', 'mayir', 'pottai',
  'punda', 'pundai', 'pundamavan', 'koothi', 'naaye', 'thaayoli', 'kandaaroli'
];
...
export function checkLocalProfanity(text: string): ProfanityMatchResult {
  ...
  // 2. Check Sinhala Script Profanities
  ...
  // 4. Check Tamil Script Profanities
