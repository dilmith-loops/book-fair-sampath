...
export const PostBookSpotModal: React.FC<PostBookSpotModalProps> = ({
  ...
  // Sync initialBookTitle & userProfile
  useEffect(() => {
    if (initialBookTitle) {
  ...
  // Real-time multilingual text check (Sinhala, Singlish, Tamil, Tanglish, English)
  const textToCheck = `${bookName} ${modalMode === 'request' ? requestNotes : shelfLocationNote} ${priceOrOffer} ${finderName} ${customStallName}`;
  const localViolation = checkLocalProfanity(textToCheck);
    ...
    try {
      ...
      for (const file of filesToProcess) {
        ...
        try {
          ...
          if (!modData.isClean) {
            setAiBlockedReason(`Sampath AI Image Shield: Photo rejected. ${modData.reason || 'Image violates safe community guidelines (inappropriate, offensive gesture, or adult content detected).'}`);
            continue;
          }
        } catch {
          // If network error during pre-vetting, proceed to main upload
        }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/75 backdrop-blur-md overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl border border-zinc-200 w-full max-w-xl shadow-[0_20px_60px_rgba(0,0,0,0.35)] my-auto overflow-hidden">
        ...
        <div className="px-4 sm:px-6 pt-3 pb-1 flex items-center justify-between text-[11px] font-bold text-zinc-500 border-b border-zinc-100 bg-zinc-50/50">
          <div className="flex items-center gap-1.5 text-[#075E54]">
            <ShieldCheck className="w-3.5 h-3.5 text-[#25D366]" />
            <span>Sampath AI Shield: Multilingual Safety & Photo Vetting Active</span>
          </div>
          <span className="text-[10px] text-zinc-400">EN • සිං • தமிழ்</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          {!localViolation.isClean && (
            <div className="p-3 bg-rose-50 border-2 border-rose-300 rounded-2xl flex items-start gap-2.5 text-xs text-rose-950 animate-in fade-in">
              ...
              <div>
                <strong className="font-black text-rose-950">Content Restricted (Marked as Profanity):</strong>
                <p className="mt-0.5 font-medium">{localViolation.reason}</p>
                <p className="mt-1 text-[10px] text-rose-700">
                  Sampath Bank PLC community standards strictly forbid profanity, nonsensical/gibberish text, racist slurs, and communal disharmony.
                </p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full px-3.5 py-2.5 bg-zinc-50 border border-zinc-300 rounded-xl text-sm font-bold text-zinc-900 focus:outline-none focus:ring-2 focus:ring-[#F37021] focus:bg-white transition-all shadow-inner"
            />
          </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 bg-zinc-50 border border-zinc-300 rounded-xl text-xs font-medium text-zinc-900 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all"
                />
              </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-2">
                  {images.map((img, idx) => (
                    <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border-2 border-zinc-300 bg-zinc-100 group shadow-xs">
                      ...
                      <span className="absolute bottom-1 left-1 bg-black/70 text-emerald-300 text-[9px] font-black px-1.5 py-0.5 rounded flex items-center gap-0.5">
