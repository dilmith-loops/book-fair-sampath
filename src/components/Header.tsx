        ...
        <div className="flex items-center gap-2 sm:gap-2.5 flex-shrink-0">
          ...
          {onOpenAdmin && (
            <button
              onClick={onOpenAdmin}
              id="header-admin-btn"
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-zinc-900 text-[#F37021] hover:bg-zinc-800 flex items-center justify-center font-black text-sm shadow-xs transition-all cursor-pointer flex-shrink-0 border border-zinc-700"
          ...
          <button
            ...
            title={userProfile?.name ? `${userProfile.name} - Profile` : 'User Profile & Registration'}
          >
            {userProfile?.name ? (
